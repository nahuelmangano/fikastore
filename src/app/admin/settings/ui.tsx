...
export default function AdminSettingsPage({
  ...
  siteTitle: string;
  faviconUrl: string;
  mpStatus: MercadoPagoConnectionStatus;
  ...
  const [mpConnection, setMpConnection] = useState(mpStatus);
  ...
  async function refreshMercadoPagoStatus() {
    const res = await fetch("/api/admin/mercadopago/oauth/status");
    const data = await res.json().catch(() => ({}));
    if (res.ok && data?.status) setMpConnection(data.status);
  }
  ...
  async function connectMercadoPago() {
    ...
    const res = await fetch("/api/admin/mercadopago/oauth/start", { method: "POST" });
    const data = await res.json().catch(() => ({}));
    setMpLoading(false);

    if (!res.ok || !data?.authorizationUrl) {
      setMpMsg(String(data?.error || "No se pudo iniciar la conexion."));
      return;
    }
  ...
  async function disconnectMercadoPago() {
    ...
    const res = await fetch("/api/admin/mercadopago/oauth/status", { method: "DELETE" });
    const data = await res.json().catch(() => ({}));
    setMpLoading(false);

    if (!res.ok) {
      setMpMsg(String(data?.error || "No se pudo desconectar Mercado Pago."));
      return;
    }
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-base font-semibold">Mercado Pago OAuth</h2>
              <p className="mt-1 text-sm text-zinc-400">
                Conexion usada para crear preferencias y consultar pagos por webhook.
              </p>
            </div>
            ...
            <div
              className={`rounded-full px-3 py-1 text-xs font-semibold ${
                mpConnection.connected
                  ...
                  : mpConnection.fallbackEnvToken
                    ? "bg-amber-500/10 text-amber-200"
                    : "bg-red-500/10 text-red-200"
              }`}
            >
              {mpConnection.connected ? "OAuth conectado" : mpConnection.fallbackEnvToken ? "Usando .env" : "Sin conectar"}
            </div>
          </div>

          <div className="mt-5 grid gap-3 text-sm text-zinc-300 sm:grid-cols-2">
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3">
              <div className="text-xs text-zinc-500">Cuenta Mercado Pago</div>
              <div className="mt-1">{mpConnection.userId || "Sin datos"}</div>
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3">
              <div className="text-xs text-zinc-500">Vencimiento del token</div>
              <div className="mt-1">
                {mpConnection.expiresAt ? new Date(mpConnection.expiresAt).toLocaleString("es-AR") : "Sin datos"}
              </div>
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3">
              <div className="text-xs text-zinc-500">Modo</div>
              <div className="mt-1">
                {mpConnection.liveMode === null ? "Sin datos" : mpConnection.liveMode ? "Produccion" : "Prueba"}
              </div>
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3">
              <div className="text-xs text-zinc-500">Credenciales de app</div>
              <div className="mt-1">{mpConnection.clientConfigured ? "Configuradas" : "Faltan variables"}</div>
            </div>
          </div>

          <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-3 text-sm">
            <div className="text-xs text-zinc-500">Redirect URL para configurar en Mercado Pago</div>
            <div className="mt-1 break-all text-zinc-200">
              {mpConnection.redirectUri || "/api/admin/mercadopago/oauth/callback"}
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={mpLoading || !mpConnection.clientConfigured}
              className="rounded-xl bg-zinc-100 px-4 py-2 text-sm font-semibold text-zinc-900 hover:bg-white disabled:opacity-50"
            >
              {mpLoading ? "Procesando..." : mpConnection.connected ? "Reconectar" : "Conectar Mercado Pago"}
            </button>
            ...
            {mpConnection.connected && (
              <button
                ...
                disabled={mpLoading}
                className="rounded-xl border border-red-900/40 bg-red-900/20 px-4 py-2 text-sm text-red-200 hover:bg-red-900/30 disabled:opacity-50"
              >
                Desconectar
              </button>
            )}
          ...
          {!mpConnection.clientConfigured && (
            <div className="mt-4 rounded-xl border border-amber-900/40 bg-amber-900/10 p-3 text-sm text-amber-100">
              Configura MP_CLIENT_ID y MP_CLIENT_SECRET para habilitar la conexion OAuth.
            </div>
          )}

          {mpMsg && <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-3 text-sm">{mpMsg}</div>}
        </section>
