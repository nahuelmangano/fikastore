...
import { getFaviconUrl, setFaviconUrl } from "@/lib/storeSettings";
...
function isImageFile(file: File) {
  return file.type.startsWith("image/") || file.name.toLowerCase().endsWith(".ico");
}
...
export async function POST(req: Request) {
  ...
  const filename = `${crypto.randomUUID()}-favicon-${safeName(file.name)}`;
  const filepath = path.join(uploadsDir, filename);
  await writeFile(filepath, bytes);

  const faviconUrl = `/uploads/${filename}`;
  await setFaviconUrl(faviconUrl);
